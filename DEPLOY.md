...
## ⚠️ Đã deploy từ trước — cập nhật đợt mới nhất (xoá tài khoản demo, Người chịu trách nhiệm nhập tay, Báo cáo cho có "Khác")

1. Vào Supabase → **SQL Editor** → mở file **`db/migration_004_cleanup_demo.sql`**, copy
   toàn bộ, dán vào → **Run** (chọn "Run and enable RLS" nếu được hỏi).
   - File này **xoá vĩnh viễn** 7 tài khoản demo: Supervisor Picking, Đại diện khách hàng,
     Leader Đóng gói, Leader Nhận hàng, Nguyễn Văn A, Trần Thị B, **Phạm Quang**.
   - **Giữ lại** `admin@3pl.local` làm Admin dự phòng.
   - Việc/dữ liệu do tài khoản THẬT khác tạo ra (nếu anh đã tạo tài khoản thật qua trang
     Quản lý người dùng) **không bị ảnh hưởng** — chỉ tự động gỡ về "chưa gán" nếu trước
     đó đang được 1 trong 7 tài khoản demo này xử lý.
   - Đã kiểm thử kỹ với dữ liệu giả lập trước khi giao, an toàn để chạy trên dữ liệu thật.
2. **Quan trọng:** Sau khi chạy xong, `admin@3pl.local` là tài khoản duy nhất còn lại.
   Đăng nhập ngay (`admin@3pl.local` / `Admin@123`) và **đổi mật khẩu** (trang Đổi mật khẩu),
   sau đó tạo các tài khoản thật (email công ty) qua trang Quản lý người dùng.
3. Đẩy code mới lên GitHub như bình thường (`git add . && git commit -m "update" && git push`).

## ⚠️ Đã deploy từ trước và muốn cập nhật bản mới nhất (Bộ phận phân cấp, Vị trí mới, Báo cáo cho theo chức danh, Bảng tin nội bộ...)?
...
1. Vào Supabase → **SQL Editor** → mở file **`db/migration_003_reorg.sql`** trong source
   code mới nhất, copy toàn bộ nội dung, dán vào → **Run** (chọn "Run and enable RLS"
   nếu được hỏi).
   - File này đã được kiểm thử kỹ: **giữ nguyên toàn bộ user và work item đã có**.
   - Riêng **Bộ phận** và **Vị trí** của các user/work item cũ sẽ bị đưa về "chưa phân
     loại" (vì danh sách cũ bị xoá hoàn toàn theo đúng yêu cầu thay đổi) — sau khi chạy
     xong, cần vào lại từng user/work item để gán lại Bộ phận/Vị trí theo danh sách mới.
   - Nếu trước đó anh **chưa từng chạy** `db/migration_002_positions.sql`, cần chạy file
     đó **trước** rồi mới chạy `migration_003_reorg.sql` (đúng thứ tự 002 → 003).
...
(file `db/schema.sql` đã bao gồm sẵn mọi thứ mới nhất).

## ⚠️ Đã deploy từ trước và muốn cập nhật bản trước đó (thêm vị trí, Gán cho tôi...)?
